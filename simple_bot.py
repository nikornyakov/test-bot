...
async def send_welcome_message(bot, group_id):
    ...
    try:
        welcome_text = """
👋 Добро пожаловать в группу баскетбольных тренировок! 🏀

Я - бот, который поможет организовать тренировки. Вот что я умею:

📅 *Автоматические опросы*:
- По понедельникам в 12:00 создаю опрос на тренировку во вторник
- По средам в 12:00 создаю опрос на тренировку в четверг

⏰ *Напоминания*:
- По вторникам и четвергам утром отправляю напоминание о тренировке
...
💡 *Как пользоваться*:
Просто участвуйте в опросах - отмечайте, придете ли вы на тренировку. 
Опросы неанонимные, так что все видят, кто планирует прийти.

По всем вопросам обращайтесь к администраторам группы.
